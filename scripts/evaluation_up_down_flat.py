...
def calculate_direction_accuracy(actual_prices, predicted_prices, flat_threshold=0.001):
    """
    Calculate accuracy based on price movement direction.
    
    Parameters:
    - actual_prices: numpy array of actual prices
    - predicted_prices: numpy array of predicted prices
    - flat_threshold: threshold below which price change is considered flat
    ...
    # Determine directions (1: up, 0: flat, -1: down)
    actual_directions = np.zeros_like(actual_changes)
    actual_directions[actual_changes > flat_threshold] = 1
    actual_directions[actual_changes < -flat_threshold] = -1
    
    predicted_directions = np.zeros_like(predicted_changes)
    predicted_directions[predicted_changes > flat_threshold] = 1
    predicted_directions[predicted_changes < -flat_threshold] = -1
    ...
    # Calculate accuracy for each movement type
    up_indices = actual_directions == 1
    down_indices = actual_directions == -1
    flat_indices = actual_directions == 0
    
    up_accuracy = (predicted_directions[up_indices] == 1).sum() / max(1, up_indices.sum())
    down_accuracy = (predicted_directions[down_indices] == -1).sum() / max(1, down_indices.sum())
    flat_accuracy = (predicted_directions[flat_indices] == 0).sum() / max(1, flat_indices.sum())
    
    # Count occurrences of each direction
    up_count = up_indices.sum()
    down_count = down_indices.sum()
    flat_count = flat_indices.sum()
...
if __name__ == "__main__":
    ...
    #dataloader_list = [val_loader, test_loader]
    #dataloader_list = [test_loader]
    ...
    for key, dataloader, c in zip(titles, dataloader_list, colors):
        ...
        # Add direction accuracy calculation
        direction_results = calculate_direction_accuracy(targets, preds)
        direction_txt = f"Direction Accuracy: {direction_results['overall_accuracy']:.4f} (Up: {direction_results['up_accuracy']:.4f}, Down: {direction_results['down_accuracy']:.4f}, Flat: {direction_results['flat_accuracy']:.4f})"
        print_and_write(f, direction_txt)
